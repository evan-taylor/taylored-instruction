...
function mapCsvRowToProduct(csvRow) {
  const price = Number.parseFloat(csvRow["Regular price"]);
  if (Number.isNaN(price)) {
    console.warn(
      `Skipping row with invalid price: ID=${csvRow.ID}, Name=${csvRow.Name}, Price='${csvRow["Regular price"]}'`
    );
  ...
  return {
    original_csv_id: Number.parseInt(csvRow.ID, 10) || null,
    sku: csvRow.SKU || null,
    name: csvRow.Name,
  ...
  Papa.parse(csvFileContent, {
    ...
    complete: async (results) => {
      ...
      const productsToInsert = results.data
        ...
        .filter((product) => product?.name); // Filter out skipped rows and rows without names

      console.log(
        `Attempting to insert ${productsToInsert.length} valid products.`
      );
      ...
      try {
        ...
        for (let i = 0; i < productsToInsert.length; i += BATCH_SIZE) {
          ...
          if (error) {
            ...
            console.log(
              `Successfully inserted batch ${Math.floor(i / BATCH_SIZE) + 1} (${data?.length || 0} rows).`
            );
          }
        }
