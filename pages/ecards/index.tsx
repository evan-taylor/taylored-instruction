...
const ECardsPage: NextPage = () => {
    ...
    const fetchECardsAndPrices = async () => {
      ...
      try {
        ...
        const { data: supabaseProducts, error: fetchError } = await supabase
          ...
          ) // Ensure stripe_price_id is selected
          .eq("type", "ecard");
        ...
        if (priceIds.length === 0) {
          ...
          ); // Fallback, ideally handle better
          setIsLoading(false);
          return;
        }
        ...
        const enrichedProducts = supabaseProducts.map((supaProduct) => {
          ...
          if (stripeInfo && stripeInfo.unit_amount !== null) {
            ...
            console.warn(
              `Stripe price info not found or invalid for ${supaProduct.stripe_price_id}. Product: ${supaProduct.name}`
            );
  ...
  if (loading || isLoading) {
    return (
      <div className="flex flex-col min-h-screen">
        <Head>
          <title>eCards | Taylored Instruction</title>
          <meta
            name="description"
            content="Purchase digital certification eCards for CPR, BLS, First Aid, and other safety training courses."
          />
        </Head>
        <Header />
  ...
  if (!session) {
    return (
      <div className="flex flex-col min-h-screen">
        <Head>
          <title>eCards - Access Denied | Taylored Instruction</title>
          <meta
            name="description"
            content="You must be logged in to purchase eCards."
          />
        </Head>
        <Header />
        <main className="flex-grow container mx-auto px-4 py-8 flex items-center justify-center">
  ...
  if (!isInstructor) {
    return (
      <div className="flex flex-col min-h-screen">
        <Head>
          <title>
            eCards - Instructor Access Required | Taylored Instruction
          </title>
          <meta
            name="description"
            content="Only approved instructors can purchase eCards."
          />
        </Head>
        <Header />
        <main className="flex-grow container mx-auto px-4 py-8 flex items-center justify-center">
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Head>
        <title>Purchase eCards | Taylored Instruction</title>
        <meta
          name="description"
          content="Purchase digital certification eCards for CPR, BLS, First Aid, and other safety training courses."
        />
